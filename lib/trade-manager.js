...
class TradeManager {
  constructor() {
    ...
    this.epochLength = Config.getEpochLength();
  }
  ...
  async registerAccount(ethAddress) {
    ...
    if (shieldedAccount) {
      ...
      try {
        ...
        logger.info(`Successfully registered shielded account ${shieldedAccount} with ZSC contract ${this.zsc}`);
      } catch (err) {
        logger.error(`Error while calling register() on ZSC contract ${this.zsc}. ${err}`);
  ...
  async mint(receiver, amount) {
    ...
    try {
      ...
      logger.error(`Error while calling mint() on token contract ${this.erc20}. ${err}`);
  ...
  async approveZSC(ethAddress, amount) {
    ...
    try {
      ...
      logger.error(`Error while calling approve() on token contract ${this.erc20}. ${err}`);
  ...
  async _fundAccount(ethAddress, amount, shieldedAccount) {
    ...
    try {
      ...
      logger.error(`Error while calling fund() on zsc ${this.zsc}. ${err}`);
  ...
  async withdraw(ethAddress, amount) {
    ...
    const epoch = Math.floor(new Date().getTime() / 1000 / this.epochLength);
    const contract = new this.web3.eth.Contract([func], this.zsc);
    let shieldedAccountState;
    logger.info(`Retrieving state of shielded account with ZSC ${this.zsc}`);
    try {
    ...
    logger.info(`Generating proof, this might take some time ${this.zsc}`);
    let result;
    try {
    ...
    await this._withdraw(withdrawArgs);
  }

  async _withdraw(withdrawArgs) {
    logger.info(`Withdrawal of shielded tokens in progress`);
    const func = _locateFunctionOrEvent(zscABI, 'burn');
    const args = [withdrawArgs.y, withdrawArgs.value, withdrawArgs.u, withdrawArgs.proof];
    try {
      ...
      const receipt = await this.sendTransaction(this.zsc, withdrawArgs.sender, func, args);
      logger.info(`withdrawal of shielded tokens was successful: (transactionHash: ${receipt.transactionHash})`);
    } catch (err) {
      logger.error(`Error while calling burn() on zsc ${this.zsc}. ${err}`);
  ...
  async transfer(zsc, senderAddress, receiverAddress, transferValue, decoys) {
    ...
    zsc = zsc.toLowerCase();
    const func = _locateFunctionOrEvent(ZSC_ABI, 'simulateAccounts');
    const epoch = Math.floor(new Date().getTime() / 1000 / this.epochLength);
    const web3 = await this.serviceutil.getRPCConnection();
    const contract = new web3.eth.Contract([func], zsc);
    let accountStates;
    ...
    const shieldedAccounts = await this.walletManager.wallets[WALLET_TYPE.SHIELDED_LOCAL].getAccounts();
    const senderAccount = shieldedAccounts.find((sa) => sa.account.isSameAddress(senderAddress));
    if (!senderAccount) {
  ...
  async signWithThrowAwayAccount(func, args, zsc) {
    ...
    const signingAccount = await this.generateThrowAwayAccount();
  ...
  async _transfer(zsc, transferArgs) {
    ...
    const func = _locateFunctionOrEvent(ZSC_ABI, 'transfer');
    const args = [transferArgs.L, transferArgs.R, transferArgs.y, transferArgs.u, transferArgs.proof];
    try {
      const response = await this.signWithThrowAwayAccount(func, args, zsc);
  ...
  async _getBalance(shieldedAccount, epoch) {
    const func = _locateFunctionOrEvent(zscABI, 'simulateAccounts');
    const contract = new this.web3.eth.Contract([func], this.zsc);
    const pubKey = bn128.serialize(shieldedAccount.address);
    logger.info(`Retrieving balance of shielded account ${pubKey} with ZSC ${this.zsc}`);
    let result;
    try {
      result = await contract.methods.simulateAccounts([pubKey], epoch).call();
  ...
  async getBalance(shieldedAddress) {
    ...
    const epoch = Math.floor(new Date().getTime() / 1000 / this.epochLength);
    return await this._getBalance(shieldedAccount.account, epoch + 1);
  }

  async generateThrowAwayAccount() {
    const index = this.signingAccountIndex++;
    const mnemonic = this.hdWallet.secret;
    logger.info(`Mnemonic ${mnemonic}`);
    const account = await Generator.generateNodes(mnemonic, [index])[0];
