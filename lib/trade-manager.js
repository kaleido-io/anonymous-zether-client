...
const { getLogger, HttpError, getEVMRevertMessage } = require('./utils');
...
class TradeManager {
  ...
  async registerAccount(ethAddress) {
    ...
    if (shieldedAccount) {
      ...
      try {
        ...
        logger.error(`Error while calling register() on ZSC contract. ${err}`);
  ...
  async fundAccount(ethAddress, amount) {
    ...
    try {
      ...
      logger.error(`Error while calling fund() on zsc. ${err}`);
  ...
  async withdraw(ethAddress, amount) {
    ...
    try {
      ...
      logger.error(`Error while calling burn() on zsc. ${err}`);
  ...
  async transfer(senderAddress, receiverAddress, transferValue, decoys) {
    ...
    try {
      ...
      ////////////
      logger.error(`Error while calling transfer() on zsc. ${err}`);
