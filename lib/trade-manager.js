...
class TradeManager {
  ...
  async withdraw(ethAddress, amount) {
    ...
    try {
      ...
      await this._handleTxError(func, args, ethAddress, err.receipt.blockNumber);
  ...
  async _handleTxError(func, args, from, blockNumber) {
    ...
    try {
      ...
      await getEVMRevertMessage(params, this.web3, blockNumber);
    } catch (err) {
      logger.error(err);
    }
  }
