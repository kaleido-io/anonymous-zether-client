...
class Prover {
  ...
  isSameAddress(address) {
    return Array.isArray(address) && address.length === 2 && this.address[0] === address[0] && this.address[1] === address[1];
  }
  ...
  async generateProof(payload) {
    ...
    if (payload.type === ZKP_PROOF_TYPE.TRANSFER_PROOF) {
      try {
        let { anonSet, anonSetStates, value, index, randomness, balanceAfterTransfer, epoch } = payload.args;
      ...
      try {
        let { burnAccount, burnAccountState, value, balanceAfterTransfer, epoch, sender } = payload.args;
  ...
  _generateTransferProof(anonSet, anonSetStates, transferValue, index, randomness, balanceAfterTransfer, epoch) {
    ...
    var data = {};
    data.proof = ZetherProver.prove(statement, witness).serialize();
    data.L = C.map((ciphertext) => bn128.serialize(ciphertext.left()));
    data.R = bn128.serialize(R);
    data.u = bn128.serialize(bn128Utils.u(epoch, witness['sk']));
    return { data: data };
  }
  ...
  _generateBurnProof(burnAccount, burnAccountState, burnValue, balanceAfterTransfer, epoch, sender) {
    ...
    const deserialized = ElGamal.deserialize(burnAccountState[0]);
    // calculate the encrypted amount after applying the burn value
    var Cn = deserialized.plus(new BN(-burnValue));
