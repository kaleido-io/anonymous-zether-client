...
class CashTokenClient extends BaseClient {
  constructor(walletManager) {
    ...
    this.erc1155 = Config.getERC1155Address();
    if (!this.erc1155) {
      throw new Error('Must provide the address of the ERC1155 contract');
    }
  ...
  async approve(ethAddress, spender, amount) {
    logger.info('Approving the ZSC contract as spender in ERC20');
    ...
    try {
      ...
      throw new HttpError(`Failed to approve the ZSC contract as spender for the account ${ethAddress} in ERC20`, 500);
    }
  }

  //
  // setApprovalForAll method in contract ERC1155.sol
  //
  async approveErc1155(ethAddress, spender) {
    logger.info('Approving the ZSC contract as spender in ERC1155');
    const func = locateFunctionOrEvent(erc1155ABI, 'setApprovalForAll');
    const args = [spender, true];
    try {
      const receipt = await this.sendTransaction(this.erc1155, ethAddress, func, args);
      logger.info(`Successfully approved the ZSC contract for owner ${ethAddress} in token ${this.erc1155} (transactionHash: ${receipt.transactionHash})`);
    } catch (err) {
      await this.handleTxError(func, args, ethAddress, err);
      throw new HttpError(`Failed to approve the ZSC contract as spender for the account ${ethAddress} in ERC1155`, 500);
    }
  }
