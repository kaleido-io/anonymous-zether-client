...
class ZetherTokenClient extends BaseClient {
  ...
  async registerAccount(ethAddress, name) {
  ...
  async fundAccount(ethAddress, amount) {
    ...
    await this.cashTokenClient.approve(ethAddress, this.zsc, amount);
  ...
  async transfer(senderAddress, receiverAddress, transferValue, decoys) {
    const { result, shuffledAccounts } = await this.prepareTransfer(senderAddress, receiverAddress, transferValue, decoys);
  ...
  async prepareTransfer(senderAddress, receiverAddress, transferValue, decoys) {
    decoys = decoys || [];
