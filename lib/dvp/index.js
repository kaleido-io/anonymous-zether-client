...
class DvPClient extends BaseClient {
  ...
  async decodeDvPStartedEvent(data, eventDef) {
    ...
    if (address) {
      ...
      logger.info(`Decoded "DvpStarted" event parties has my shielded account, attempting to decrypt the amount ${encryptedAmount}`);
      // const transferValue = await this.tradeManager.zetherTokenClient._decryptEncryptedBalance(encryptedAmount, myAccount.account);
