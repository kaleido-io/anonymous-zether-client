...
class DvPClient extends BaseClient {
  ...
  async startDvP(senderAddress, receiverAddress, transferValue) {
    ...
    try {
      ...
      const response = await this.sendTransaction(this.dvp, oneTimeAccount.address, func, args);
      logger.info(`startDvp successful: (transactionHash: ${response.transactionHash})`);
      return { txHash: response.transactionHash, txSubmitter: oneTimeAccount.address, proof: result.proof };
    } catch (err) {
      await this.handleTxError(func, args, oneTimeAccount, err);
  ...
  async executeDvP(senderEthAddress, counterpartyEthAddress, proof) {
    ...
    try {
      const response = await this.sendTransaction(this.dvp, senderEthAddress, func, args);
