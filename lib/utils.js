...
const { ZSC_EPOCH_LENGTH } = require('./constants.js');
...
function numberToHex(payload, property, required) {
  if ((payload[property] || required) && typeof payload[property] === 'number') {
    try {
      ...
      logger.error(`Failed to convert payload.${property} ${payload[property]} to hex`, err);
...
function shuffleAccountsWParityCheck(accounts, sender, receiver) {
  ...
  if (m == 0 || (m & (m - 1)) != 0) {
    ...
    logger.err(`Number of accounts in shuffle should be a power of 2`);
  ...
  if (!match(accounts[senderIndex], sender)) {
    logger.err(`Sender address is not in the accounts array`);
    throw new Error(`Sender address is not in the accounts array`);
  }

  if (!match(accounts[receiverIndex], receiver)) {
    logger.err(`Receiver address is not in the accounts array`);
...
function timeBeforeNextEpoch() {
  ...
  return Math.ceil(current / ZSC_EPOCH_LENGTH) * ZSC_EPOCH_LENGTH - current;
}
module.exports = {
  HttpError,
  getLogger,
