...
class Convolver {
  ...
  constructor() {
    ...
    const fft = (input, omegas, inverse) => {
      // crazy... i guess this will work for both points and scalars?
    ...
    this.convolution = (exponent) => {
      // returns only even-indexed outputs of convolution!
      const size = exponent.length();
      const temp = base_fft.hadamard(fft(exponent.flip(), omegas, false));
      return fft(
        temp
          .slice(0, size / 2)
          .add(temp.slice(size / 2))
          .times(Convolver.two_inv),
        inverses,
        true
      );
